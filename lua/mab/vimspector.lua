...
function M.generate_debug_profile()
  ...
  if ft == "python" then
    ...
    vim.notify("Unsupported language - " .. ft, vim.log.levels.INFO, { title = "Generate Debug Profile" })
  end
end
...
function M.toggle_human_mode()
  if vim.g.vimspector_enable_mappings == nil then
    ...
    vim.notify("Enabled HUMAN mappings", vim.log.levels.INFO, { title = "Debug" })
  else
    vim.g.vimspector_enable_mappings = nil
    vim.notify("Disabled HUMAN mappings", vim.log.levels.INFO, { title = "Debug" })
  end
end
